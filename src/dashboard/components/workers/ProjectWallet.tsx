    ...
    >
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 10 }}>
        <div style={{
          ...
          boxShadow: hov ? `0 0 10px ${card.color}30` : 'none', transition: 'box-shadow 0.25s',
        }}>
  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '0 12px', borderRight: '1px solid rgba(255,255,255,0.06)', height: '100%', flexShrink: 0 }}>
        <div style={{ width: 5, height: 5, borderRadius: '50%', background: ACCENT, boxShadow: `0 0 6px ${ACCENT}`, animation: 'pw-dot-pulse 1.5s ease-in-out infinite' }} />
...
function AIActionBanner({
  ...
  if (availableToWithdraw > 0) {
    message = `Payment approved — ${fmtFull(availableToWithdraw)} ready for immediate release. Initiate transfer now.`;
    signal = 'approved by GMG team · ACH ready · funds secured';
    ctaLabel = 'Get Paid →';
  } else if (safe?.status === 'pending') {
    message = 'ACH transfer in progress — funds settling within 1–2 business days. No action needed.';
    signal = 'transfer confirmed · T+2 ACH settlement · no action needed';
    ctaLabel = 'Track Status';
  } else if (safe?.status === 'delayed') {
    message = 'Payment processing delayed. Your funds are fully secured — team is resolving the upstream hold.';
    ...
    message = 'Submit final deliverables to unlock payment. Compliance is clear — funds are staged and ready to release.';
    signal = 'deliverables pending · compliance verified · payment staged';
    ctaLabel = 'Submit Now';
  } else {
    message = 'All deliverables submitted. Final review in progress — payment released once approved by project team.';
    signal = 'deliverables under review · payment release pending team approval';
    ctaLabel = 'Check Status';
  }

  if (actioned) {
    message = 'Action submitted. The GMG project team has been notified and will process your request shortly.';
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 7, marginBottom: 3 }}>
            <span style={{ fontFamily: 'monospace', fontSize: 7, fontWeight: 800, letterSpacing: '0.13em', textTransform: 'uppercase' as const, color: actioned ? 'rgba(16,185,129,0.55)' : 'rgba(236,72,153,0.55)' }}>
              {actioned ? 'Submitted' : 'Best Move Right Now'}
            </span>
        ...
        {!actioned && (
          <div style={{ display: 'flex', gap: 7, flexShrink: 0 }}>
            <button style={{ fontFamily: 'monospace', fontSize: 8, padding: '6px 11px', borderRadius: 8, cursor: 'pointer', background: 'transparent', border: '1px solid rgba(255,255,255,0.08)', color: 'rgba(255,255,255,0.2)' }}>
              Dismiss
            </button>
            <button
...
function AIInsightStack({ worker, availableToWithdraw }: { worker: WorkerWithRelations; availableToWithdraw: number }) {
  ...
  if (availableToWithdraw > 0) {
    insights.push({
      ...
      text: `${fmtFull(availableToWithdraw)} approved and ready — initiate transfer to receive funds within 24–48 hours.`,
  ...
  if (missing.length > 0) {
    insights.push({
      ...
      text: `${missing.length} compliance item${missing.length > 1 ? 's' : ''} outstanding — required before payment can be released.`,
  ...
  if (safe?.status === 'pending') {
    insights.push({
      ...
      text: 'ACH transfer in progress. Payments typically settle within 1–2 business days after initiation.',
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 7 }}>
      {insights.map((insight, i) => (
        <div key={i} style={{
          ...
          background: `${insight.color}06`, border: `1px solid ${insight.color}18`,
        }}>
...
function CurrentProjectPanel({ worker, onGetPaid }: { worker: WorkerWithRelations; onGetPaid: () => void }) {
  ...
  return (
    <div style={{ position: 'relative', background: 'rgba(255,255,255,0.022)', border: `1px solid ${ACCENT}18`, borderRadius: 13, overflow: 'hidden' }}>
    ...
    <div style={{ background: 'rgba(255,255,255,0.022)', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 13, overflow: 'hidden' }}>
      ...
      <div>
        {worker.assignments.map((item, i) => {
          ...
          return (
            <div key={item.id} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 16px', borderBottom: i < worker.assignments.length - 1 ? '1px solid rgba(255,255,255,0.03)' : 'none' }}>
              ...
              <span style={{ fontFamily: 'monospace', fontSize: 7.5, padding: '3px 7px', borderRadius: 5, color: meta.color, background: `${meta.color}12`, border: `1px solid ${meta.color}22`, flexShrink: 0 }}>{meta.label}</span>
            </div>
          );
        })}
      </div>
    ...
    <div style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 13, overflow: 'hidden' }}>
      ...
      {paidSafes.length === 0 ? (
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '32px 16px', gap: 10 }}>
          ...
          <p style={{ fontSize: 11, textAlign: 'center', color: 'rgba(255,255,255,0.2)', margin: 0 }}>No completed projects yet. Your payment history will appear here.</p>
        </div>
      ) : (
        <>
          {visible.map((s, i) => {
            ...
            return (
              <div
                key={s.id}
                style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '11px 16px', borderBottom: i < visible.length - 1 ? '1px solid rgba(255,255,255,0.03)' : 'none', transition: 'background 0.15s' }}
    ...
    <div style={{ position: 'relative', overflow: 'hidden', borderRadius: 13, background: 'rgba(245,158,11,0.03)', border: '1px solid rgba(245,158,11,0.2)' }}>
      ...
      <div style={{ padding: '12px 16px', display: 'flex', flexDirection: 'column', gap: 10 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'flex-start', gap: 8, borderRadius: 10, padding: '10px 14px', background: 'rgba(245,158,11,0.04)', border: '1px solid rgba(245,158,11,0.1)' }}>
          ...
          <p style={{ fontSize: 10.5, lineHeight: 1.6, color: 'rgba(255,255,255,0.32)', margin: 0 }}>Upstream payment delay. Funds will be released once received. Your payment is fully secured.</p>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '80px 16px', gap: 24 }}>
      ...
      <p style={{ fontSize: 10, color: 'rgba(255,255,255,0.18)' }}>
        Questions? Email{' '}
        <a href="mailto:projects@greatermusic.com" style={{ color: `${ACCENT}55` }}>projects@greatermusic.com</a>
      </p>
    </div>
  );
}
...
interface ProjectWalletProps {
  member: IndustryOSMember;
  system?: WorkerSystem;
}

export default function ProjectWallet({ member, system = 'industry_os' }: ProjectWalletProps) {
  ...
  const cards: WalletCardDef[] = [
    ...
    },
    {
      ...
      cta: availableToWithdraw > 0 ? { label: 'Get Paid', onClick: () => setShowModal(true) } : undefined,
    },
  ];
  ...
  return (
    <>
      <style>{`
        @keyframes pw-dot-pulse { 0%,100%{opacity:1;box-shadow:0 0 4px ${ACCENT}} 50%{opacity:.5;box-shadow:0 0 10px ${ACCENT}} }
        @keyframes pw-pulse-ring { 0%{transform:scale(1);opacity:.7} 100%{transform:scale(2.2);opacity:0} }
        @keyframes pw-glow { 0%,100%{opacity:.5} 50%{opacity:1} }
        @keyframes pw-float { 0%,100%{transform:translateY(0)} 50%{transform:translateY(-3px)} }
        @keyframes spin { from{transform:rotate(0deg)} to{transform:rotate(360deg)} }
      `}</style>

      {/* ══ OUTER SHELL — exact CampaignWallet structure ══ */}
      <div style={{
        ...
        <div style={{ padding: '18px 22px 22px' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
              <div style={{ position: 'relative', width: 40, height: 40, flexShrink: 0 }}>
                ...
                <div style={{
                  ...
                  animation: pulse ? 'pw-pulse-ring 1.4s ease-out forwards' : 'none',
                  opacity: 0,
                }} />
              </div>
              <div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 3 }}>
                  <h2 style={{ fontSize: 15, fontWeight: 800, color: '#F0F0F2', letterSpacing: '-0.02em', margin: 0 }}>Project Wallet</h2>
                  ...
                  <div style={{
                    ...
                    <div style={{ width: 5, height: 5, borderRadius: '50%', background: ACCENT, boxShadow: `0 0 5px ${ACCENT}`, animation: 'pw-dot-pulse 1.5s ease-in-out infinite' }} />
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '5px 12px', borderRadius: 20, background: `${pColor}10`, border: `1px solid ${pColor}25` }}>
          ...
          {/* ── CURRENT PROJECT ── */}
          <div style={{ marginBottom: 12 }}>
            <CurrentProjectPanel worker={worker} onGetPaid={() => setShowModal(true)} />
          </div>
